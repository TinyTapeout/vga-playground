...
class AudioResamplerProcessor extends AudioWorkletProcessor {
  ...
  handleMessage(event) {
    ...
    if (data.type === 'samples') {
      ...
      const samplesAvailable =
        (this.writeIndex - this.readIndex + this.ringBufferSize) % this.ringBufferSize;
  ...
  process(inputs, outputs) {
    ...
    const samplesRequired =
      Math.round(outputData.length * playbackRate * this.downsampleFactor) + borderSamples;
    ...
    if (samplesAvailable < borderSamples) {
      for (let i = 0; i < outputData.length; i++) outputData[i] = this.previousSample;
    ...
    for (let i = 0; i < this.downsampleBuffer.length; i++) {
      ...
      this.downsampleBuffer[i] = this.interpolate(
        this.ringBuffer,
        (this.readIndex + intPos) % this.ringBufferSize,
        (this.readIndex + nextIntPos) % this.ringBufferSize,
        frac,
      );
    }
    ...
    if (N > 1) {
      for (let i = 0; i < outputData.length; i++) {
        ...
        for (let j = 1; j < N; j++) acc += this.downsampleBuffer[i * N + j];
        outputData[i] = acc / N;
      }
    } else {
      for (let i = 0; i < outputData.length; i++) outputData[i] = this.downsampleBuffer[i];
    }
